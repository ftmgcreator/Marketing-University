...
use App\Models\User;
use App\Services\AdmissionImporter;
...
class ProcessAdmissionImport implements ShouldQueue
{
    ...
    public function handle(AdmissionImporter $importer): void
    {
        ...
        try {
            ...
            $this->notifyUser(true, sprintf(
                "%s — %d ta yangi qo'shildi, %d ta yangilandi, %d ta o'tkazildi (%d qator).",
                $this->originalName,
                $stats['created'],
                $stats['updated'],
                $stats['skipped'],
    ...
    private function notifyUser(bool $success, string $message): void
    {
        ...
        $notification = Notification::make()
            ->title($success ? 'Talabalar import qilindi' : 'Talabalarni import qilishda xatolik')
            ->body($message);
